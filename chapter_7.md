...
The transfer problem can be formulated as finding a policy (mathematical expression) that performs well in the real environment (mathematical expression) using knowledge from a simulated environment (mathematical expression):

(Mathematical optimization expression)

subject to having access only to samples from (mathematical expression).

<!-- ![Figure 7.1: The Sim-to-Real Transfer Problem](/img/physical-ai/sim-to-real-transfer.svg) -->
<!-- TODO: Create sim-to-real-transfer.svg image for this chapter -->
