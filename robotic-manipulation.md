...
<!-- TODO: Create humanoid-lab.svg image for this chapter -->
...
![Robotic Manipulation Concept](/img/undraw_docusaurus_mountain.svg)
...
![Atlas Robot Manipulation](/img/undraw_docusaurus_react.svg)
...
W equals the set of all sums from i=1 to n of f_i such that f_i is an element of C_i

where f_i represents the contact force at the i-th contact point and C_i is the friction cone at that contact.
...
|f_t| is less than or equal to mu * f_n

The friction cone represents all possible contact forces that can be applied at a contact point without causing slip:

C equals the set of all f such that f_t^2 + f_n^2 is less than or equal to mu^2 * f_n^2
...
Q_min = minimum over w in boundary W of ||w||

**L1 Norm Metric:** Measures the maximum wrench that can be resisted along each axis:

Q_L1 = minimum over i of (sum over j of |w_ij|)
...
where x is the end-effector pose and q is the vector of joint angles.
...
where J is the Jacobian matrix and J^dagger is its pseudo-inverse.

For dynamic modeling, the equation of motion for a manipulator is given by:

M(q)q_double_dot + C(q, q_dot)q_dot + g(q) = tau + J^T * f_ext

where M is the mass matrix, C contains Coriolis and centrifugal terms, g is the gravity vector, tau is the joint torque vector, and f_ext represents external forces.
...
The manipulability ellipsoid is defined as the set of all vectors v such that v-transpose times J-inverse-transpose times J-inverse times v equals 1

The volume of this ellipsoid, given by omega equals the square root of the determinant of J times J-transpose, indicates the manipulator's ability to move in all directions equally.
...
maximize the sum from i=1 to n of w_i * s_i
subject to: n_i^T(p_j - p_i) >= epsilon, for all i,j

where w_i are weights, s_i are stability scores, n_i are surface normals, and p_i are contact points.
...
P(success|I, g) = CNN(I, g)

where I is the input image and g represents the grasp pose parameters.
...
J(pi) = expected value of [sum from t=0 to T of gamma^t * (r_t + alpha * H(pi(dot|s_t)))]
...
minimize with respect to u_0:H-1 of sum from k=0 to H-1 of l(x_k, u_k) + l_f(x_H)
subject to: x_k+1 = f(x_k, u_k)
u_min is less than or equal to u_k is less than or equal to u_max
...
where v is visual information, t is tactile feedback, and p is proprioceptive data.
...
minimize with respect to theta of the expected value over (s,a) sampled from D of [L(pi_theta(s), a)]

where D is the demonstration dataset, and L is a loss function measuring the difference between predicted and demonstrated actions.
...
p(g|o) = integral of p(g_T|o) times the product from t=1 to T of p_t(g_t-1|g_t, o) dg_1:T
...
<!-- TODO: Create optimus-folding.svg image for this chapter -->
