...
Formally, safety can be defined as ensuring the system remains within a set of safe states (mathematical expression):

(Mathematical expression)

where (mathematical expression) is the system state at time (mathematical expression).

<!-- ![Figure 10.1: Safety Framework for Physical AI Systems](/img/physical-ai/safety-framework.svg) -->
<!-- TODO: Create safety-framework.svg image for this chapter -->

## 10.2 Formal Methods for Safety

### 10.2.1 Control Barrier Functions

Control Barrier Functions (CBFs) provide a mathematical framework for ensuring safety constraints in control systems. A function (mathematical expression) is a Control Barrier Function if:

There exists an α in class K_∞ such that sup over u in U of [∇h(x)ᵀf(x) + ∇h(x)ᵀg(x)u + α(h(x))] ≥ 0

where (mathematical expression) and (mathematical expression) define the system dynamics (mathematical expression).
