---
title: Chapter 6 – Reinforcement Learning in Physical Domains
sidebar_position: 6
---
...
The core RL framework consists of an agent interacting with an environment through actions and receiving observations and rewards. The goal is to learn a policy that maximizes the expected cumulative reward.

Here is the mathematical formula for expected cumulative reward:

```text
J(pi) = E[sum from t=0 to T of gamma^t * r(s_t, a_t)]
```

where gamma is the discount factor between 0 and 1.

<!-- ![Figure 6.1: Reinforcement Learning Loop in Physical Systems](/img/physical-ai/rl-loop-physical-systems.svg) -->
<!-- TODO: Create rl-loop-physical-systems.svg image for this chapter -->
...
Policy gradient methods directly optimize the policy parameters by following the gradient of the expected return. The REINFORCE algorithm implements this approach:
...
Safe RL can be formulated as a constrained optimization problem where the goal is to maximize expected reward while satisfying safety constraints.
